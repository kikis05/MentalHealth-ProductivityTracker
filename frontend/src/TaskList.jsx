...
const TaskList = ({ taskItems, updatesByTask, updateCallback }) => {
  ...
  const increaseDays = async (id) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:5000/increase_task_counter/${id}`,
        options,
      );
      if (response.status === 200) {
        updateCallback();
  ...
  const onSubmitUpdate = async (taskId) => {
    ...
    if (date == "") {
      console.log("Date isn't working: " + d.toUTCString().substring(0, 16));
    }
    const newData = {
      taskId,
      date,
    ...
    const response = await fetch(
      `http://127.0.0.1:5000/create_update/${taskId}`,
      options,
    );
    if (response.status !== 201 && response.status !== 200) {
      const error_message = await response.json();
  ...
  return (
    <div className="task-info-container">
      <div className="task-header">
        {listObject == null ? (
          <p>
            You're doing great! For each task, click on the pin to see details.
          </p>
        ) : (
          updatesByTask.map((t) =>
      ...
      <div className="task-board">
        {taskItems.map((task) => (
          <div className="task-item" key={task.id}>
            <button
              className="task-pin"
              style={{ background: task.color, margin: "2px" }}
              onClick={(e) => {
                setTaskBasedUpdate(task.taskName);
              }}
            ></button>
            <p>
            ...
            <button onClick={() => onDelete(task.id)} className="task-complete">
              Task Complete
            </button>
          </div>
        ))}
